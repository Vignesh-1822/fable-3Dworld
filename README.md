...
dawn"* and Claude translates it into engine parameters; the world rebuilds in
front of you.
...
```
 ...
 └────────│─────────────────────────────────────────────────────────┘
          ...
          ▼
   AWS Lambda ──▶ Claude API (structured outputs → validated JSON)
```
...
- **AI layer** — Claude structured outputs → zod-validated, range-clamped `WorldParams`; free mock mode for local dev
...
Add `ANTHROPIC_API_KEY=sk-ant-…` before the dev-server command for real
...
shadcn/ui · TanStack Query · zod · Vite · AWS Lambda · Claude API
